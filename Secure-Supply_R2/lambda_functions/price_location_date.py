...
def lambda_handler(event, context):
    ...
    # Extracting date and location from the original document
    date = None
    location = None
    for key, value in kvs.items():
        if "Date of Invoice:" in key:
            date = value[0]
        elif "Delivered to:" in key:
            location = value[0]
    
    # Write data to CSV file
    csv_data = [['Date of Invoice', date], ['Delivered to', location], ['Unit Price', unit_price]]
    ...
    return {
        'statusCode': 200,
        'body': json.dumps({'Date of Invoice': date, 'Delivered to': location, 'Unit Price': unit_price}),
        's3_bucket': bucket,
        's3_key': s3_key
    }
